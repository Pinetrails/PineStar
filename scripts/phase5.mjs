#!/usr/bin/env node
// phase5.mjs - StarNet Hermes replacement-readiness loop.
...
function evidenceHealth() {
  ...
  const strictReady = !!(w.passed
    && browser.status === 'hermes-proven'
    && computer.status === 'hermes-proven'
...
function checkWorkloadBaseline(loop) {
  const file = join(ROOT, 'docs', 'STARNET_PHASE5_HERMES_WORKLOADS.md');
  const text = readText(file).toLowerCase();
  const required = ['ready-to-replace', 'contract-green is not hermes-proven', 'live model', 'browser', 'computer-use', 'desktop'];
  const missing = required.filter(term => text.indexOf(term) < 0);
  return {
    id: '5.1-hermes-workload-baseline',
    phase: '5.1',
    title: 'Hermes workload baseline is explicit',
    loop,
...
function ensurePhase4PilotDecision() {
  ...
  writeJson(file, {
    ...
    acceptedPilotGaps: [
      'Browser/computer-use still need P5 Hermes-proven workload evidence.',
...
function deriveGaps(ev) {
  ...
  if (!browser || browser.status !== 'hermes-proven') gaps.push('Browser automation is ' + ((browser && browser.status) || 'missing') + ', not Hermes-proven by a live replacement workload.');
  if (!computer || computer.status !== 'hermes-proven') gaps.push('Desktop computer-use is ' + ((computer && computer.status) || 'missing') + ', not Hermes-proven by an attended driver workload.');
...
function checkDecision(loop, priorResults) {
  ...
  if (priorBad.length) {
    return {
      ...
      phase: '5.7',
      title: 'Final Hermes replacement decision',
      loop,
      status: 'blocked',
      required: true,
      class: 'decision',
...
async function runOnce(loop) {
  ...
  if (WANT_LIVE) {
    ...
    const workload = await commandStep({
      ...
      phase: '5.2',
      title: 'Live StarNet UI Hermes-style workload',
      cmd: npmCmd,
...
function writeSummary(allResults, loopsRun) {
  ...
  md += 'Run `npm.cmd run phase5:loop` or `npm.cmd run phase5:live` after each fix or evidence update. Use `npm.cmd run phase5:ready` when checking whether HermesAgent can be fully replaced.\n\n';
  md += '## Next Action\n\n';
  ...
  else if (replacementReady) md += 'P5 is ready-to-replace green. StarNet can replace HermesAgent as the main harness.\n';
