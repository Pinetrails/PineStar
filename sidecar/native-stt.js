...
const { randomUUID: randomUUIDDefault } = require('node:crypto');
...
// Standard click-to-talk records one bounded take in the browser. Feed that exact take to the same local
// recognizer instead of letting System.Speech decide the endpoint from a pause. The path is generated by the
// sidecar, passed out-of-band in the child environment (never interpolated into the command), and removed in
// finally below.
const WINDOWS_WAV_SCRIPT = [
  "$ErrorActionPreference='Stop'",
  'Add-Type -AssemblyName System.Speech',
  "[Console]::OutputEncoding=[Text.Encoding]::UTF8",
  '$r=New-Object System.Speech.Recognition.SpeechRecognitionEngine',
...
function makeNativeStt(opts) {
  ...
  const fsp = opts.fsp || fspDefault;
  const tempDir = typeof opts.tempDir === 'string' ? opts.tempDir : os.tmpdir();
  const randomUUID = opts.randomUUID || randomUUIDDefault;
  ...
  function runRecognition(script, signal, audioPath) {
    return new Promise(resolve => {
      ...
      execFile('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], options, (error, stdout) => {
        ...
        if (!error) return resolve({ ok: true, text: '' });
        ...
        resolve({ ok: timedOut, text: '', error: timedOut ? 'no-speech' : 'native speech failed' });
      });
    });
  }
  ...
  async function recognize(params) {
    params = params || {};
    if (!status().available) return { ok: false, text: '', error: 'native speech is unavailable on this platform' };
    const wav = params.audioWav;
    if (!wav || !wav.length) return runRecognition(WINDOWS_SCRIPT, params.signal);
...
module.exports = { WINDOWS_SCRIPT, WINDOWS_WAV_SCRIPT, NOISE_TOKENS, DEFAULT_MIN_CONFIDENCE, parseRecognition, gateTranscript, makeNativeStt };
