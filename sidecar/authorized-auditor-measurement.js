...
function sealedRunOptions(spec) { return { key:'', model:spec.model, provider:PLAN.provider, configurationId:spec.configurationId, fallbackModels:[], fallbackProviders:[], keyPool:[], isTask:false, reflect:false, emit:function(){} }; }
function claimAttempt(fs, receiptFile, body) { const fd=fs.openSync(receiptFile,'wx'); try { fs.writeFileSync(fd,JSON.stringify(body,null,2)); fs.fsyncSync(fd); } finally { fs.closeSync(fd); } }
...
module.exports={PLAN,PLAN_DIGEST,canonical,sha,instruction,expected,parseModel,sealedRunOptions,claimAttempt,validateStart,execute};
