/* sidecar/permgrants.js — the STANDING-GRANT manager behind the Permissions Panel (autonomy Stage B / B1).
   ...
   makeGrantManager({ grantsPermanent, persist, grantable }) -> { snapshot, grant, revoke }
     snapshot()      -> { grants:[dangerKey...], grantable:[dangerKey...] }   // grants = EVERYTHING blessed (honest)
...
(function (root, factory) {
  ...
  function makeGrantManager(opts) {
    ...
    function snapshot() {
      return { grants: sortedGrants(), grantable: Array.from(grantable).sort() };
    }

    // ADD a standing grant — only a curated, local-safe key. Persist-before-commit (fail-closed).
    function grant(key) {
    ...
    // "revokes" a grant that's still on disk.
    function revoke(key) {
      if (typeof key !== 'string' || !key) return { ok: false, reason: 'missing key', grants: sortedGrants() };
      if (!grantsPermanent.has(key)) return { ok: true, grants: sortedGrants() };   // idempotent
