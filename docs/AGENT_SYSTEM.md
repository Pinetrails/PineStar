...
`PS-2026-015` bridges recurring definitions into StarNet's existing cron scheduler. A definition is stored only as a protected `cron.jobs.json` record whose bounded metadata identifies the owning system role and objective template; it has no unattended grants and local-only delivery. Enabling requires exactly one approved runtime identity bound to the capable role. Cron remains authoritative for parsing, next fire, durable advance-before-run, locks, pause, E-stop/halt, leases, cancellation, retry, recovery, and outcome telemetry. At a fire, the driver delegates to one durable objective occurrence and does not also call cron's `runOnce`; that occurrence uses normal routing, admission, activation, and settlement. Same-fire retries and shifted retries reuse an unresolved occurrence, while a settled prior occurrence permits the next scheduled instance. Protected templates produce `approval_required` records and never auto-approve or activate. An explicit `open-source-scout` workflow reuses the PS-2026-014 directive and structured report marker.

`PS-2026-016` deterministically aggregates durable objectives, Scout reports, Auditor outcomes, approval backlog, and measured run-store costs into the existing bounded shared-report contract. It makes no model call and omits claims for which no durable evidence exists.

`PS-2026-017` lets a user explicitly queue a safe assigned/admitted objective for Away work. The queue is durable and bounded, but it is not another scheduler: the existing Night Shift driver retains presence, unattended-posture, leash, cooldown, concurrency, persistence-before-fire, and E-stop authority. A cleared beat atomically claims one queued objective and sends it through normal admission, role/runtime binding, activation, cancellation, and settlement. Protected objectives cannot enter the queue; transient pre-activation retries stop after three claims and stale claims are recoverable after one hour.

`PS-2026-050` adds the first advisory champion/challenger evaluator. An authenticated request names two different configurations and disjoint cohorts of settled objectives assigned to the same stable system role. It derives bounded completion, evidence, failure, cancellation, measured-cost, uncertain-mutation, and wasted-work proxies from objective/run history, stores an immutable shared report, and returns a conservative recommendation. Missing run evidence is explicit. The evaluator cannot admit or activate a configuration, alter routing/roles/models/permissions, retire a configuration, or spend.
