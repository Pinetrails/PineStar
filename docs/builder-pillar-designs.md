...
interface SaveDoc { schema:'skynet.station'; version:number; station:StationSerialized; }
...
- **world/bus.ts (event bridge)** — Emits the new `worldChange` event (carrying WorldPatch) on U.bus, plus keeps re-emitting v7's existing names so agents.js/ui.js/render.js listeners are untouched. This is the seam the Skynet bridge plugs real-agent events into. _(reuses: The entire U.bus emit/on contract and every existing event name (chat, sale, parcel, deliverable, task, stats, level, ...).)_
