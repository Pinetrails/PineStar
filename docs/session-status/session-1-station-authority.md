...
Safe checkpoint: Station-derived routing authority. `makeStationStore()` now compiles a routing plan from the accepted `SaveDoc.station`, and `router.resolveTarget()` prefers that server-derived plan over any legacy `/api/routing` plan posted by the renderer.
