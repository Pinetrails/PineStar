...
| 2026-09-07 | Accept `retain_champion_no_candidate` for `auditor-matched-local-v2-repeated-v1`. | Retain `operations-auditor.ollama-llama3.2-3b.v1` without treating its 0/9 result as adequate quality; keep the challenger inactive and create no candidate because its narrow roster/format advantage did not establish overall operational superiority. |
| 2026-09-07 | Deterministic mechanical checks are authoritative for exactly computable audit facts. | Auditor models may interpret, prioritize, summarize, express uncertainty, and recommend follow-up, but cannot override JSON validity, counts, invariants, reference integrity, roster/configuration joins, or other directly computable facts. |
