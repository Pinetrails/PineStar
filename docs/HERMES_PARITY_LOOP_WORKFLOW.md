...
| 5. State safety | Hermes takes pre-update/pre-migration full-state backups and has targeted cron snapshot restore paths so protected jobs are not silently lost. | Closed: cron routines now use the same resilient JSON helper as other protected stores, snapshotting `cron.jobs.json.bak` before durable replace and recovering it on torn/corrupt boot. | Green: `node test/cron.durability.test.js` and `node test/cron.api.test.js` cover protected routine recovery. |
