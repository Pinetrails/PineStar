...
- **Your overseer sits in its chair again.** It was landing half a tile to the side of the
  seat and reading as though it were standing on the desk rather than working at it. The chair
  was always in the right place; the body wasn't reaching it. Walking to a desk needs a whole
  tile to aim at, but a chair on an even-width desk is drawn on the centre line *between* two
  tiles — so an overseer that walked to its desk stopped on the tile and never stepped the last
  six pixels onto the chair. Crew were always doing this correctly; the overseer is the one
  body that took a different route to its seat, which is why it was the only one that looked
  wrong.
- **A delegating overseer no longer sits there doing nothing while its own run is live.** When
  a task handed work to a summoned worker, the permission request pulled the overseer away from
  its desk — and the trip *back* was zero distance, because the desk it was sent to wait at was
  its own. A zero-length walk never finishes, so the overseer never resumed working: it sat
  motionless for the entire run while COMMS and the crew panel both correctly said WORKING.
  Truthful telemetry has to cut both ways — the world may no more show idle over a live run
  than a panel may show work over a dead one.
...
- **macOS: download the installer manually this one time.** In-app updating has never actually
  ...
  This release publishes them, so from v0.6.8 onward the Mac updater works like the Windows one.
