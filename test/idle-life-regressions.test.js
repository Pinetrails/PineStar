...
A.ok(/self\.pendSeat\s*=\s*\{\s*px:\s*\(sx\s*\+\s*0\.5\)\s*\*\s*T\s*\+\s*\(side\s*\?\s*side\.dx\s*:\s*0\),\s*py:\s*\(couch\.y\s*\+\s*h\)\s*\*\s*T\s*-\s*2\s*\}/.test(world), 'couch planning records the claimed cushion render position (offset only for a profile seat)');
