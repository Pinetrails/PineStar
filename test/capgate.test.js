...
// ---- A. resolveTools (pure projection) ----
{
  ...
  A.eq(full.tools.slice().sort(), ['notebook.feedback', 'notebook.read', 'notebook.write', 'quest.update', 'recall_conversation', 'skill.list', 'skill.manage', 'skill.view', 'skill.write', 'todo', 'tool.search', 'widget.set'], 'notebook grants its tools; compute excluded from tools[]; quest.update + tool.search ride the COMPUTER placed in this same room');
  ...
  // The bare compute-only office: both freebies ride the computer object so no surface is ever without them —
  // quest.update because the prompt commands its use, tool.search because a deferred tool must stay reachable.
  A.eq(compOnly.tools.slice().sort(), ['quest.update', 'tool.search'], 'computer-only room grants exactly the two computer freebies, no notebook tools');
