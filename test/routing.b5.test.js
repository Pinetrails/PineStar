...
// connector manager can project THAT server's tools. resolveTools yields no STATIC grant for it (dynamic at run).
{
  ...
  A.eq(res.tools.slice().sort(), ['code.run', 'quest.update', 'tool.search'], 'a connector adds NO static tool beyond the computer primitives (its MCP tools project dynamically at run time)');
}
