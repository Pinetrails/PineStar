...
A.eq(Object.keys(f.dims).sort(), ['goals', 'identity', 'stack', 'standing_orders', 'style'], 'fresh() has all five dimensions, each empty');
...
A.eq(sm0.blank.length, 5, 'a cold dossier has five blank dimensions');
const smX = D.fresh();
D.upsert(smX, 'identity', { text: 'a' }, 1);
D.upsert(smX, 'stack', { text: 'b' }, 2);
const sm = D.summary(smX, { observed: { dominant: 'code' } });
A.eq(sm.familiarity, 2 / 5, 'familiarity = fraction of dimensions with at least one belief');
