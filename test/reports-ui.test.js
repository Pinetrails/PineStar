...
const review = Reports.publicationReviewHtml([{ project: { id: 'p<script>', title: 'Trail & Kit', listingDraft: { title: '<Listing>', description: 'Ready & reviewed', tags: ['trail<script>'], seoKeywords: ['hike&plan'] } } }], [{ id: 'objective:<1>', status: 'approval_required', approvalState: 'required', protectedAction: true, classification: { workflow: 'product-publication-approval', projectId: 'p<script>', requestId: 'market&v1', qaReportId: 'qa:<1>', targetMarketplaces: ['Market<script>'] } }], [{ id: 'qa:<1>', type: 'product-qa', deliverableEvidence: [{ deliverable: 'Letter <PDF>', artifactId: 'artifact:&1' }] }]);
A.ok(review.includes('PROTECTED · WAITING') && review.includes('market&amp;v1') && review.includes('Trail &amp; Kit'), 'publication review shows protected identity and product truth');
A.ok(review.includes('&lt;Listing&gt;') && review.includes('Ready &amp; reviewed') && review.includes('Market&lt;script&gt;'), 'publication review safely renders listing copy and marketplace targets');
A.ok(review.includes('Letter &lt;PDF&gt;') && review.includes('artifact:&amp;1') && review.includes('QA COVERAGE'), 'publication review renders linked QA coverage');
A.ok(!/<button|onclick=|<form|href=/i.test(review) && review.includes('approval, withdrawal, publication, navigation, and execution are unavailable'), 'publication review exposes no decision or action control');
A.ok(Reports.publicationReviewHtml([], [], []).includes('No protected publication reviews'), 'empty review queue is explicit');
