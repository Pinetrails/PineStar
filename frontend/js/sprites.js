...
const SPR = (() => {
  ...
  /* screen palette per phase for animated monitors */
  const scrCols = ['#62ff9e', '#3fd07c', '#7adfb0', '#2fa863'];
  const scr = (ph) => scrCols[Math.floor((now / 700 + ph) % scrCols.length)];

  /* ============ FURNITURE ============ */
  /* detailed procedural pixel art: outlined casings, 2-3 tone shading,
     contact shadows, and animated emissive bits. same footprints + work
     behaviors as always â€” screens only light when f.work. */
  ...
  function drawFurn(f, work) {
    const X = f.x * T, Y = f.y * T, W = (f.w || 1) * T, H = (f.h || 1) * T;
    const fn = F[f.t];
    f.work = work;
    if (fn) fn(X, Y, W, H, f);
  }
  ...
  /* ============ AGENT SPRITE ============ */
  /* body: {px, py (pixel pos), color, state, dir, animT, sitting, robot, working, hasCan} */
  function drawAgent(b, selected) {
    if (typeof SPRITES !== 'undefined' && SPRITES.ready) {
      const g = SPRITES.drawBody(ctx, b, now);
      if (g) {
        const x = Math.round(b.px);
        if (selected) {
          ctx.strokeStyle = '#fff'; ctx.lineWidth = 1;
          ctx.strokeRect(x - g.w / 2 - 1.5, g.top - 1.5, g.w + 3, b.py - g.top + 3);
        }
        drawOverlays(b, x, g.top);
        return;
      }
    }
    drawProcedural(b, selected);
  }

  /* alert / speech-bubble overlays shared by both body renderers */
  function drawOverlays(b, x, ty) {
    if (b.alert) {
      const ay = ty - 9 + Math.sin(now / 200) * 1;
      px(x - 1, ay, 3, 4, '#ffd34a'); px(x, ay + 1, 1, 2, '#1a1a00');
      ctx.fillStyle = '#ffd34a'; ctx.font = '8px monospace'; ctx.fillText('!', x + 3, ay + 5);
    }
    if (b.bubble && b.bubble.until > now) {
      if (b.bubble.txt) {
        // phosphor caption: the actual line, as a tiny comm chip above the head
        ctx.font = '7px monospace';
        const tw = Math.ceil(ctx.measureText(b.bubble.txt).width);
        const bx = Math.round(x - tw / 2), by = ty - 15;
        ctx.globalAlpha = 0.82; px(bx - 3, by, tw + 6, 9, '#0c0a04'); ctx.globalAlpha = 1;
        px(bx - 3, by, tw + 6, 1, '#665522'); px(bx - 3, by + 8, tw + 6, 1, '#665522');
        px(x - 1, by + 9, 2, 2, '#665522'); // tail
        ctx.fillStyle = '#ffd34a'; ctx.fillText(b.bubble.txt, bx, by + 7);
      } else {
        const bx = x + 5, by = ty - 8;
        px(bx, by, 12, 7, '#dfe8df'); px(bx + 1, by + 1, 10, 5, '#f4f8f0');
        px(bx - 1, by + 5, 2, 2, '#dfe8df');
        const d = Math.floor(now / 250) % 3;
        for (let i = 0; i <= d; i++) px(bx + 2 + i * 3, by + 3, 2, 2, '#3a4a40');
      }
    }
  }

  function drawProcedural(b, selected) {
    const x = Math.round(b.px), y = Math.round(b.py);
    const c = b.color, dark = U.shade(c, -0.45), skin = b.robot ? '#b8b8c8' : '#e8c8a8';
    const walk = b.state === 'walk';
    const step = walk ? Math.floor(now / 140) % 2 : 0;
    const bob = b.sitting ? 0 : (walk ? 0 : Math.sin(now / 600 + b.phase) * 0.6);
    const H = b.sitting ? 11 : 14;
    const ty = y - H + bob;

    if (selected) { ctx.strokeStyle = '#fff'; ctx.lineWidth = 1; ctx.strokeRect(x - 5.5, ty - 3.5, 11, H + 4); }

    // shadow
    ctx.globalAlpha = 0.3; px(x - 4, y - 1, 8, 2, '#000'); ctx.globalAlpha = 1;

    if (!b.sitting) {
      // legs
      px(x - 3, y - 4, 2, 4 - (step ? 1 : 0), dark);
      px(x + 1, y - 4 + (step ? 0 : 0), 2, 4 - (step ? 0 : 1), dark);
    }
    // torso
    px(x - 4, ty + 4, 8, b.sitting ? 5 : 6, c);
    px(x - 4, ty + 4, 8, 1, U.shade(c, 0.25));
    px(x - 1, ty + 5, 2, 3, dark); // chest panel
    // arms
    if (b.working && blink(160, b.phase)) { px(x - 5, ty + 5, 1, 3, c); px(x + 4, ty + 6, 1, 3, c); }
    else { px(x - 5, ty + 5, 1, 4, dark); px(x + 4, ty + 5, 1, 4, dark); }
    // beer can in hand
    if (b.hasCan) { px(x + 5, ty + 4 + (blink(1400, b.phase) ? 0 : 1), 2, 3, '#8a98a8'); px(x + 5, ty + 4 + (blink(1400, b.phase) ? 0 : 1), 2, 1, '#caa84a'); }
    // head
    px(x - 3, ty, 6, 4, skin);
    px(x - 3, ty - 1, 6, 1, b.robot ? '#88889a' : '#3a2e22'); // hair / crown
    if (b.robot) { px(x - 2, ty + 1, 4, 1, blink(600) ? '#ff4a3d' : '#7a1a14'); } // visor
    else { px(x - 2, ty + 1, 1, 1, '#222'); px(x + 1, ty + 1, 1, 1, '#222'); }
    // headset antenna
    px(x + 3, ty - 2, 1, 2, dark); px(x + 3, ty - 3, 1, 1, blink(800, b.phase) ? c : dark);

    // status icons (shared overlay renderer)
    drawOverlays(b, x, ty);
  }

  /* belt parcels (cosmetic items riding conveyors) */
  function drawParcel(p) {
    px(Math.round(p.x) - 3, Math.round(p.y) - 4, 6, 5, '#36424c');
    px(Math.round(p.x) - 2, Math.round(p.y) - 3, 4, 1, '#caa84a');
  }

  return {
    setCtx(c) { ctx = c; },
    setNow(t) { now = t; },
    drawFurn, drawAgent, drawParcel, px: (...a) => px(...a), blink
  };
})();
