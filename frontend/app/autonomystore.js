...
const AutonomyStore = (() => {
  ...
  function init() { state = ready() ? Autonomy.normalize(load()) : (load() || floor()); }
  ...
  // the WRITERS (the awakening cadence beat + the dial panel). Each commits + persists and returns the new posture.
  function applyPreset(id) { if (!ready()) return get(); state = Autonomy.applyPreset(get(), id); save(); return get(); }
  function setInitiative(level) { if (!ready()) return get(); state = Autonomy.setInitiative(get(), level); save(); return get(); }
  function setReach(level) { if (!ready()) return get(); state = Autonomy.setReach(get(), level); save(); return get(); }
  function setLeash(n) { if (!ready()) return get(); state = Autonomy.setLeash(get(), n); save(); return get(); }
