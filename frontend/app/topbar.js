...
const Topbar = (() => {
  ...
  function paintSig() {
    ...
    if (!ls || !ls.bridged || ls.paused) {
      ...
      el.title = ls && ls.paused ? 'connection to the background service is paused (disconnected)' : 'connection to Pine Star’s background service — standby (not connected yet)';
      return;
    }
    if (ls.down) {
      ...
      el.title = 'lost the connection to Pine Star’s background service — live numbers pause until it’s back (is the app still running?)';
    } else {
      el.classList.remove('down', 'standby');
      el.childNodes[0].nodeValue = 'UPLINK ';
      bars.textContent = SIG_UP;
      el.title = 'connected to Pine Star’s background service — everything on screen is live';
    }
  }
