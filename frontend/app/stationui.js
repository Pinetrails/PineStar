  ...
  function renderAgentSkills(host, skills, agentId) {
    ...
    for (const s of sorted) {
      ...
      const provenance = s.sourceUrl
        ? '<div class="sk-prov"><span>EXCHANGE SOURCE</span><a href="' + esc(s.sourceUrl) + '" target="_blank" rel="noopener noreferrer">' + esc(s.sourceUrl) + '</a></div>' +
          '<div class="sk-prov"><span>SOURCE SHA-256</span><code>' + esc(s.sourceDigest || 'unknown') + '</code></div>'
        : '';
      const updateBtn = s.sourceUrl && s.state !== 'archived'
      ...
      html +=
        '<div class="sk-card ' + state + (held ? ' held' : '') + '" data-agent-skill="' + esc(s.id) + '">' +
          ...
          '</div>' +
          '<div class="sk-body">' + heldBlock + provenance + '<pre>' + esc(s.body || '') + '</pre>' + files + absorbed +
            '<div class="sk-update-slot"></div>' +
            '<div class="consent-btns mc-acts">' +
              heldBtn +
              updateBtn +
    ...
    host.querySelectorAll('[data-ag-act]').forEach(btn => btn.addEventListener('click', async () => {
      ...
      if (act === 'check') {
        const slot = card.querySelector('.sk-update-slot');
        btn.classList.add('busy'); btn.textContent = 'CHECKING…';
        const r = await Harness.skillExchangeCheck({ agentId, id: skill.id });
        btn.classList.remove('busy'); btn.textContent = 'Check update';
        if (r && r.ok && r.preview) {
          if (r.preview.updateAvailable) renderSkillExchangePreview(slot, r.preview, agentId, { update: true });
          else if (slot) slot.innerHTML = '<div class="sk-exchange-note ok">UP TO DATE — the source digest still matches this installed skill.</div>';
        } else if (slot) slot.innerHTML = '<div class="sk-exchange-note bad">' + esc((r && r.error) || 'Could not check this source.') + '</div>';
        return;
      }
