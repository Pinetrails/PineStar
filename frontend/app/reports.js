...
(function (root, factory) {
  ...
  function reportHtml(row) {
    const r = row || {}, discoveries = Array.isArray(r.discoveries) ? r.discoveries : [], deliverableEvidence = Array.isArray(r.deliverableEvidence) ? r.deliverableEvidence : [], experimentAdvisories = Array.isArray(r.experimentAdvisories) ? r.experimentAdvisories : [];
    ...
    const advisoryHtml = experimentAdvisories.length ? '<section><h4>EXPERIMENT-PLANNING ADVISORIES</h4>' + experimentAdvisories.map(x => '<div class="ps-discovery"><strong>' + esc(x && x.question) + '</strong>' + list('DESIGN REQUIREMENTS', x && x.designRequirements) + (x && x.sourceRefs && x.sourceRefs.length ? '<div class="dim">EVIDENCE · ' + x.sourceRefs.map(esc).join(' · ') + '</div>' : '') + '<p class="muted">Inspection only · no configuration, cohort, schedule, rerun, or objective control is available here.</p></div>').join('') + '</section>' : '';
    return '<article class="cfg-card ps-report"><div class="dim">' + esc(String(r.type || 'report').toUpperCase()) + ' · ' + esc(dateText(r.createdAt)) + '</div><h3>' + esc(r.headline || 'Untitled report') + '</h3>' + discoveryHtml + coverageHtml + advisoryHtml + list('COMPLETED', r.completed) + list('EXCEPTIONS', r.exceptions) + list('DECISIONS', r.decisions) + list('NEXT ACTIONS', r.nextActions) + (r.sourceRefs && r.sourceRefs.length ? '<div class="dim">SOURCES · ' + r.sourceRefs.map(esc).join(' · ') + '</div>' : '') + '</article>';
  }
  ...
  function publicationReviewHtml(productEntries, objectiveRows, reportRows) {
    const products = (Array.isArray(productEntries) ? productEntries : []).map(x => (x && x.project) || x).filter(Boolean), reports = Array.isArray(reportRows) ? reportRows : [];
    const pending = (Array.isArray(objectiveRows) ? objectiveRows : []).filter(x => x && x.status === 'approval_required' && x.approvalState === 'required' && x.protectedAction && x.classification && x.classification.workflow === 'product-publication-approval');
    if (!pending.length) return '<p class="muted">No protected publication reviews are currently waiting.</p>';
    return pending.map(o => { const c = o.classification || {}, p = products.find(x => x.id === c.projectId) || {}, draft = p.listingDraft || {}, qa = reports.find(x => x && x.id === c.qaReportId && x.type === 'product-qa'), coverage = qa && Array.isArray(qa.deliverableEvidence) ? qa.deliverableEvidence : [], markets = Array.isArray(c.targetMarketplaces) ? c.targetMarketplaces : [];
      return '<article class="cfg-card ps-publication-review"><div class="dim">PROTECTED · WAITING · REQUEST ' + esc(c.requestId || 'unknown') + '</div><h3>' + esc(p.title || o.title || c.projectId || 'Publication review') + '</h3>' +
        '<div class="dim">OBJECTIVE · ' + esc(o.id || '') + ' | QA REPORT · ' + esc(c.qaReportId || 'unavailable') + '</div>' +
        (markets.length ? '<div class="dim">TARGETS · ' + markets.map(esc).join(' · ') + '</div>' : '') +
        (draft.title ? '<h4>LISTING TITLE</h4><p>' + esc(draft.title) + '</p>' : '') + (draft.description ? '<h4>LISTING DESCRIPTION</h4><p>' + esc(draft.description) + '</p>' : '') +
        list('TAGS', draft.tags) + list('SEO KEYWORDS', draft.seoKeywords) + (coverage.length ? '<section><h4>QA COVERAGE</h4><ul>' + coverage.map(x => '<li>' + esc(x && x.deliverable) + ' · ARTIFACT ' + esc(x && x.artifactId) + '</li>').join('') + '</ul></section>' : '<p class="muted">Linked QA coverage is unavailable in the current report window.</p>') +
        '<p class="muted">Inspection only · approval, withdrawal, publication, navigation, and execution are unavailable here.</p></article>'; }).join('');
  }
  ...
  async function mount(body) {
    ...
    try {
      ...
      const publicationHost = body.querySelector('#ps-publication-reviews'); if (publicationHost) publicationHost.innerHTML = publicationReviewHtml(productRows, objectiveRows, rows);
    ...
    } catch (_) { for (const id of ['#ps-reports', '#ps-publication-reviews', '#ps-product-portfolio', '#ps-product-projects', '#ps-commerce-records', '#ps-business-ledger', '#ps-objectives', '#ps-roles']) { const host = body.querySelector(id); if (host) host.innerHTML = '<p class="bad">Operational records are temporarily unavailable.</p>'; } }
  }
  return { mount, reportHtml, productProjectHtml, publicationReviewHtml, productPortfolioHtml, commerceRecordHtml, businessLedgerHtml, businessExportBundle, objectiveHtml, roleHtml, dateText, exportBundle, markdown };
});
